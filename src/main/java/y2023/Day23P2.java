...
public class Day23P2 {
    ...
    static final HashSet<Vertex> SEEN = new HashSet<>();
    ...
    static long findDistance(Vertex point, Vertex target, Map<Vertex, List<Vertex>> graph){
        ...
        for(var v: graph.computeIfAbsent(point, LC)){
            Vertex nextPoint = new Vertex(v.point[0], v.point[1]);
            if(SEEN.contains(nextPoint)){
                continue;
            }
            distance = Math.max(distance, v.point[2] + findDistance(nextPoint, target, graph));
        }
        SEEN.remove(point);
